    ...
    <section className="relative py-10 md:py-16 lg:py-20">
      <div className="container relative mx-auto px-4 md:px-6">
        <div className="mb-4 text-center md:mb-8">
          <h2 className="text-lg font-bold text-foreground sm:text-lg md:text-3xl lg:text-[2.25rem]"><Titulo t={hosts.titulo} /></h2>
          {hosts.subtitulo && <p className="mx-auto mt-3 max-w-2xl text-sm text-muted-foreground md:mt-6 md:text-base">{hosts.subtitulo}</p>}
        </div>
        <div className={`mx-auto grid grid-cols-1 gap-6 md:gap-8 lg:gap-12 ${um ? "max-w-xl" : "max-w-4xl md:grid-cols-2"}`}>
          {hosts.itens.map((h) => (
            <div key={h.nome} className="rounded-xl border border-border bg-card p-4 pt-10 text-center md:rounded-2xl md:p-8 md:pt-12">
              ...
              <h3 className="mb-2 text-base font-bold uppercase text-primary md:text-lg">{h.nome}</h3>
