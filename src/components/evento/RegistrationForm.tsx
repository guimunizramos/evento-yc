...
export function RegistrationForm({ evento, formulario, tema }: Props) {
  ...
  const campo = "h-12 border-muted-foreground/20 bg-background text-base focus:border-primary";

  return (
    <div>
      <div className="mb-5 md:mb-6">
        <div className="flex items-baseline justify-between gap-3">
          <h3 className="text-lg font-bold text-foreground md:text-2xl">{formulario.titulo}</h3>
        ...
        {etapa === 1 && (
          <>
            <div className="space-y-1.5">
              <Label htmlFor="f-name" className="text-base text-foreground">Nome completo</Label>
              <Input id="f-name" {...register("name")} placeholder="Seu nome" className={campo} />
              {errors.name && <p className="text-xs text-destructive">{errors.name.message}</p>}
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="f-email" className="text-base text-foreground">E-mail</Label>
              <Input id="f-email" type="email" {...register("email")} placeholder="seu@email.com" className={campo} />
              {errors.email && <p className="text-xs text-destructive">{errors.email.message}</p>}
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="f-phone" className="text-base text-foreground">WhatsApp</Label>
            ...
            <div className="grid grid-cols-3 gap-3 sm:grid-cols-5">
              <div className="col-span-2 space-y-1.5 sm:col-span-4">
                <Label htmlFor="f-city" className="text-base text-foreground">Cidade</Label>
                <Input id="f-city" {...register("city")} placeholder="Ex: São Paulo" className={campo} />
                {errors.city && <p className="text-xs text-destructive">{errors.city.message}</p>}
              </div>
              <div className="col-span-1 space-y-1.5">
                <Label htmlFor="f-state" className="text-base text-foreground">Estado</Label>
                <select id="f-state" {...register("state")} defaultValue="" className="flex h-12 w-full rounded-md border border-muted-foreground/20 bg-background px-3 py-2 text-base text-foreground ring-offset-background focus:border-primary focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2">
          ...
          <>
            {([["hasLot", "Já possui terreno?"], ["hasProject", "Você já possui projeto arquitetônico?"]] as const).map(([campoNome, rotulo]) => (
              <div key={campoNome} className="space-y-2">
                <Label className="text-base text-foreground">{rotulo}</Label>
                <RadioGroup onValueChange={(v) => setValue(campoNome, v as "sim" | "nao", { shouldValidate: true })} className="flex gap-6">
                  {(["sim", "nao"] as const).map((v) => (
                    <div key={v} className="flex items-center space-x-2">
                      ...
                      <Label htmlFor={`${campoNome}-${v}`} className="cursor-pointer text-base text-foreground">{v === "sim" ? "Sim" : "Não"}</Label>
                    </div>
                  ))}
                </RadioGroup>
              ...
              <RadioGroup onValueChange={(v) => setValue("investmentAmount", v as (typeof FAIXAS)[number], { shouldValidate: true })} className="flex flex-col gap-2">
                {FAIXAS.map((f, i) => (
                  <div key={f} className="flex items-center space-x-2">
                    ...
                    <Label htmlFor={`faixa-${i}`} className="cursor-pointer text-base text-foreground">{f}</Label>
                  </div>
                ))}
              </RadioGroup>
