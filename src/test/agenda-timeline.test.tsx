...
const progressHeight = (container: HTMLElement) =>
  container.querySelector<HTMLElement>("div.bg-border > div.bg-primary")?.style.height;
...
describe("AgendaSection — timeline de scroll", () => {
  it("observa os 7 marcos e começa com a linha apagada", () => {
    ...
    expect(progressHeight(container)).toBe("0%");
  });
  ...
  it("acende progressivamente conforme cada marco entra na viewport", () => {
    ...
    expect(progressHeight(container)).toBe("57.14285714285714%");

    intersect(6);
    expect(markersLit(container)).toBe(7);
    expect(progressHeight(container)).toBe("100%");
  });
  ...
  it("mantém os 7 tópicos e o CTA da seção", () => {
    ...
    expect(screen.getByText("O que é incorporação imobiliária")).toBeInTheDocument();
    expect(screen.getByText("Arquitetura de Capital e Segurança")).toBeInTheDocument();
